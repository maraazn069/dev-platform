...
function ensureUserConfig(username, opts = {}) {
  ...
  // Cert: SELALU pakai wildcard apex *.DOMAIN (cover semua subdomain depth-1).
  // Format preview baru pakai depth-1 (<project>-<user>.DOMAIN) supaya tidak butuh cert depth-2.
  const certPath = `/etc/letsencrypt/live/${domain}/fullchain.pem`;
  const certKey = certPath.replace('fullchain', 'privkey');

  // Preview block: depth-1 subdomain, format <project>-<user>.DOMAIN atau <project>-<port>-<user>.DOMAIN
  // Regex non-greedy match project, optional port di tengah, fixed -USERNAME di akhir.
  const escDomain = domain.replace(/\./g, '\\.');
  const previewBlock = `
    # Preview project (depth-1, cover by wildcard apex *.${domain}).
    #
    # Format subdomain:
    #   <project>-${username}.${domain}              → port 3000 (default, npm run dev / next dev)
    #   <project>-<port>-${username}.${domain}       → port <port> (8000-9999, custom)
    #
    # Contoh untuk user '${username}':
    #   myapp-${username}.${domain}                  → port 3000
    #   myapp-8000-${username}.${domain}             → port 8000 (python -m http.server 8000)
    #   myapp-5173-${username}.${domain}             → port 5173 (vite dev)
    server {
        listen 443 ssl;
        http2 on;
        server_name "~^(?<project>[a-z0-9][a-z0-9_-]*?)(?:-(?<port>[3-9][0-9]{3}))?-${username}\\.${escDomain}$";
        ...
        location / {
            ...
            # Resolver penting: dynamic upstream butuh resolver.
...
# User: ${username}
# Cert: wildcard apex *.${domain} (cover semua subdomain depth-1 termasuk preview project)

server {
    listen 80;
    server_name ${username}.${domain};
    # Catch HTTP redirect — preview <project>-${username}.${domain} di-handle wildcard di nginx.conf utama.
...
`;
  ...
  if (skipReload) {
    return {
      success: true,
      message: `nginx user-conf ${username}.conf ditulis (preview depth-1 aktif via cert apex). Reload: SKIPPED (caller will reload)`
    };
  }
  const reload = reloadNginx();
  return {
    success: true,
    message: `nginx user-conf ${username}.conf ditulis (preview depth-1 aktif via cert apex). Reload: ${reload.success ? 'ok' : reload.error}`
  };
}
